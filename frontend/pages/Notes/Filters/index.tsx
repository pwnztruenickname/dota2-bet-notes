...
export default function Filters ({ onVisibleElement, isVisible, heroes, teams, onSendGamesRequest }: Props) {
          ...
          <Form.List name="setupCharacterIds">
            {fields => (
              <ShouldUpdateChecker fieldPath="setupCharacterIds">
                {({ getFieldValue }) => {
                  const heroFields = (
                    getFieldValue('setupCharacterIds') as FormValuesProps['setupCharacterIds']
                  )?.reduce<number[]>(
                    (acc, el) => (el?.heroId ? [...acc, el.heroId] : acc),
                    []
                  )
